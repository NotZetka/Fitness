...
namespace IntegrationTests
{
    public class BodyControllerTests(WebApplicationFactory<Program> factory) : BaseControllerTest(factory)
    {
        ...
        public async Task UserShouldSetHeightProperly()
        {
            ...
            var user = _context.Members.Include(x => x.BodyWeight).First(x => x.UserName == "TestBodyWeight");
            ...
            var userAfterChanges = GetNewContext().Members.Include(x => x.BodyWeight).First(x => x.UserName == "TestBodyWeight");
