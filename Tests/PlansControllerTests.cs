...
namespace IntegrationTests
{
    public class PlansControllerTests(WebApplicationFactory<Program> factory) : BaseControllerTest(factory)
    {
        ...
        //    var token = GenerateJwtToken(user.Id.ToString());
        //    _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        //    var query = new CreatePlanCommand()
        //    {
        ...
        //    var userAfterChanges = GetNewContext().Members.Include(x => x.FitnessPlans).FirstOrDefault(x => x.Id == user.Id);
        ...
        //    var userManager = scope.ServiceProvider.GetRequiredService<UserManager<AppMember>>();
        ...
        //        Public = true
        //    };
        //    var user2 = _context.Members.Include(x => x.FitnessPlansTemplates).First(x => x.UserName == "TestUserPlan2");
